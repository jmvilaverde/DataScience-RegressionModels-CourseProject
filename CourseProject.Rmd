...
wt      | Weight (lb/1000)
qsec    | 1/4 mile time
vs      | V/S -> V motor or straight motor
...
library(car)
anova(model.Initial, model.Prop, model.Complete)
```
...
Conclussion for model complete
...
> With 95% confidence, we estimate that a manual transmission results in a `r confInterval.Model.Complete[1]` to `r confInterval.Model.Complete[2]` increase in MPG comparing to use of automatic transmission for the Model Initial.

Conclussion for model proposed
...
> With 95% confidence, we estimate that a manual transmission results in a `r confInterval.Model.Prop[1]` to `r confInterval.Model.Prop[2]` increase in MPG comparing to use of automatic transmission for the Model Initial.
***
